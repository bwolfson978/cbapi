...
- [`/api/group/<group_id>/datasharing`](#apiv1groupgroupiddatasharing) Enumeration of datasharing settings
- [`/api/group/<group_id>/datasharing/<config_id>`](#apigroupgroupiddatasharingconfigid) A specific configuration of datasharing settings
...
- [`/api/v1/detect/report/binarydwell/<int:days>`](#apiv1detectreportbinarydwellintdays)
...
```
####/api/v1/group/<int:group_id>/datasharing/<int:config_id>
Enumeration, deletion, addition, and updating of group datasharing configurations

*Supports*: `GET`, `POST`, and `DELETE` for `/api/v1/group/group_id/datasharing`
*Supports*: `GET` and `POST` for `/api/v1/group/group_id/datasharing/config_id`

####Returns:

- With no config_id parameter, `GET /api/v1/group/group_id/datasharing` returns the datasharing configurations for the group with id "group_id"
- With no config_id parameter, `POST /api/v1/group/group_id/datasharing` returns the added configuration info
- With no config_id parameter, `DELETE /api/v1/group/group_id/datasharing` returns whether or not deleting all datasharing configurations was successful
- With a config_id parameter, `GET /api/v1/group/group_id/datasharing/config_id` returns the datasharing info for the configuration with id "config_id"
- With a config_id parameter, `DELETE /api/v1/group/group_id/datasharing/config_id` returns whether or not deleting the configuration was successful

A datasharing configuration has the following structure:
- `what`: type of data to be shared
- `group_id`: id of the group this setting is configured for
- `id`: configuration id
- `who`: provider to share the data with

Example:
```
`GET https://172.16.100.109/api/v1/group/1/datasharing 
...
*Supports*: `GET` and `POST` for `/api/v1/banning/blacklist`
*Supports*: `GET`, `PUT`, and `DELETE` for `/api/v1/banning/blacklist/md5`
